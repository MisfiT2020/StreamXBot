...
import com.xstream.music.utils.potoken.PoTokenGenerator
import com.xstream.music.utils.potoken.PoTokenResult
import com.xstream.music.utils.sabr.EjsNTransformSolver
import okhttp3.OkHttpClient
import okhttp3.Request
import timber.log.Timber
import com.xstream.music.data.api.AudioQuality

class PlaybackException(message: String?, cause: Throwable?, val errorCode: Int) : Exception(message, cause) {
    companion object {
...
object YTPlayerUtils {
    ...
    suspend fun playerResponseForPlayback(
        ...
        if (isUploadedTrack || playlistId?.contains("MLPT") == true) {
            Timber.tag(TAG).d("PLAYBACK_DEBUG Main player response status: ${mainPlayerResponse.playabilityStatus.status}")
            Timber.tag(TAG).d("PLAYBACK_DEBUG Playability reason: ${mainPlayerResponse.playabilityStatus.reason}")
            Timber.tag(TAG).d("PLAYBACK_DEBUG Video details: title=${mainPlayerResponse.videoDetails?.title}, videoId=${mainPlayerResponse.videoDetails?.videoId}")
            Timber.tag(TAG).d("PLAYBACK_DEBUG Streaming data null? ${mainPlayerResponse.streamingData == null}")
            Timber.tag(TAG).d("PLAYBACK_DEBUG Adaptive formats count: ${mainPlayerResponse.streamingData?.adaptiveFormats?.size ?: 0}")
        }
        ...
        for (clientIndex in (startIndex until STREAM_FALLBACK_CLIENTS.size)) {
            ...
            if (streamPlayerResponse?.playabilityStatus?.status == "OK") {
                ...
                val playbackRequestHeaders = buildPlaybackRequestHeaders(currentClient)
        ...
        if (streamPlayerResponse.playabilityStatus.status != "OK") {
            ...
            if (isUploadedTrack) {
                Timber.tag(TAG).e("PLAYBACK_DEBUG FAILURE: Playability not OK for uploaded track - status=${streamPlayerResponse.playabilityStatus.status}, reason=$errorReason")
            }
        ...
        if (isUploadedTrack) {
            Timber.tag(TAG).d("PLAYBACK_DEBUG SUCCESS: Got playback data for uploaded track - format=${format.mimeType}, streamUrl=${streamUrl.take(100)}...")
        }
        PlaybackData(
            audioConfig,
            videoDetails,
            ...
            selectedClientName ?: (usedAgeRestrictedClient ?: MAIN_CLIENT).clientName,
            selectedRequestHeaders,
        )
    }.onFailure { e ->
        Timber.tag(TAG).e(e, "PLAYBACK_DEBUG EXCEPTION during playback for videoId=$videoId: ${e::class.simpleName}: ${e.message}")
    }
    /**
    ...
    suspend fun getStreamUrl(
        ...
        return playerResponseForPlayback(videoId, playlistId, audioQuality, connectivityManager)
            .map { it.streamUrl }
    }
     ...
     */
    private fun validateStatus(url: String, requestHeaders: Map<String, String>): Boolean {
        return runCatching {
            ...
            httpClient.newCall(requestBuilder.build()).execute().use { response ->
                ...
                Timber.tag(logTag).d("Stream validation HTTP $responseCode for ${url.take(120)}...")
                responseCode == java.net.HttpURLConnection.HTTP_OK ||
                    responseCode == java.net.HttpURLConnection.HTTP_PARTIAL
            }
        }.getOrElse { error ->
            Timber.tag(logTag).d(error, "Stream validation request failed")
            false
        }
    }

    private fun buildPlaybackRequestHeaders(client: YouTubeClient): Map<String, String> {
        val headers = linkedMapOf<String, String>()
        headers["User-Agent"] = client.userAgent

        if (client.clientName in listOf("WEB", "WEB_REMIX", "WEB_CREATOR", "TVHTML5", "TVHTML5_SIMPLY_EMBEDDED_PLAYER")) {
            headers["Origin"] = YouTubeClient.ORIGIN_YOUTUBE_MUSIC
            headers["Referer"] = YouTubeClient.REFERER_YOUTUBE_MUSIC
            headers["X-Origin"] = YouTubeClient.ORIGIN_YOUTUBE_MUSIC
            YouTube.visitorData?.takeIf { it.isNotBlank() }?.let { headers["X-Goog-Visitor-Id"] = it }
        }
    ...
    private fun getSignatureTimestampOrNull(videoId: String): SignatureTimestampResult {
        return SignatureTimestampResult(null, false)
    }
