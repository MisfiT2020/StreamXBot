...
object YouTube {
    ...
    suspend fun search(query: String, filter: SearchFilter): Result<SearchResult> = runCatching {
        ...
        val shelfItems = shelves?.mapNotNull { it.musicShelfRenderer }?.flatMap { shelf ->
            shelf.contents?.getItems()?.mapNotNull { SearchPage.toYTItem(it) } ?: emptyList()
        } ?: emptyList()

        val cardItems = shelves?.mapNotNull { it.musicCardShelfRenderer }?.flatMap { shelf ->
            shelf.contents?.mapNotNull { it.musicResponsiveListItemRenderer }
                ?.mapNotNull { SearchSummaryPage.fromMusicResponsiveListItemRenderer(it) } ?: emptyList()
        } ?: emptyList()

        val allItems = (cardItems + shelfItems).distinctBy { it.id }

        SearchResult(
            items = allItems,
            continuation = shelves?.mapNotNull { it.musicShelfRenderer }?.firstOrNull { it.continuations != null }
    ...
    suspend fun subscribeChannel(channelId: String, subscribe: Boolean, params: String? = null): Result<Unit> = runCatching {
