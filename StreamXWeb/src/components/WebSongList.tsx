...
const WebSongItem = memo(
  ({
    ...
    return (
      <div className={`web-song-item${isActive ? ' is-current' : ''}${index % 2 === 1 ? ' is-odd' : ''}`} data-song-id={song._id} onClick={onActivate}>
        {hideFavoriteStar ? null : (
        ...
        <div className={`web-song-cover${showPlaying ? ' is-playing' : ''}`} aria-hidden="true">
          <img src={song.cover_url || 'https://via.placeholder.com/60'} alt="" className="web-song-cover-img" loading="lazy" />
          {showOverlay ? (
            <div className="web-song-cover-overlay">
              {showPlaying ? (
                ...
                <svg className="web-song-cover-icon" viewBox="0 0 24 24" aria-hidden="true">
                  <path
                    fill="currentColor"
        ...
        <div className="web-song-info">
          <div className="web-song-title">{song.title}</div>
          <div className="web-song-artist">{song.artist}</div>
        </div>
        <button
          className="web-song-more-btn"
          onClick={(e) => {
            e.stopPropagation()
            onMenuClick(song, e)
          }}
...
WebSongItem.displayName = 'WebSongItem'

export const WebSongList = memo(({ songs, title, loading = false, showTitle = true, hideFavoriteStar = false }: WebSongListProps) => {
  ...
  return (
    <section className="web-songlist">
      {showTitle ? <h1 className="web-songlist-title">{title}</h1> : null}
      {loading ? (
        <div className="song-loading" role="status" aria-label="Loading">
          <div className="song-list-spinner" aria-hidden="true" />
        </div>
      ) : (
        <div className="web-song-list">
                ...
                <div className="song-menu-top">
                  ...
                  <button className="song-menu-icon-btn" onClick={handlePlayNext}>
                    <svg viewBox="0 0 32 28" xmlns="http://www.w3.org/2000/svg">
                      <path
                        fill="currentColor"
                        d="M18.14 20.68c.365 0 .672-.107 1.038-.323l8.508-4.997c.623-.365.938-.814.938-1.37 0-.564-.307-.988-.938-1.361l-8.508-4.997c-.366-.216-.68-.324-1.046-.324-.73 0-1.337.556-1.337 1.569v4.773c-.108-.399-.406-.73-.904-1.021L7.382 7.632c-.357-.216-.672-.324-1.037-.324-.730 0-1.345.556-1.345 1.569v10.235c0 1.013.614 1.569 1.345 1.569.365 0 .68-.108 1.037-.324l8.509-4.997c.49-.29.796-.631.904-1.038v4.79c0 1.013.615 1.569 1.345 1.569z"
                      />
                    </svg>
                    <span>Play Next</span>
                  </button>
...
WebSongList.displayName = 'WebSongList'
